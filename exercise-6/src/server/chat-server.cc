...
tt::chat::server::ClientServer::ClientServer(int port)
...
tt::chat::server::ClientServer::~ClientServer() { close(socket_); }

void tt::chat::server::ClientServer::handle_connections() {
...
void tt::chat::server::ClientServer::set_socket_options(int sock, int opt) {
  using namespace tt::chat;
...
void tt::chat::server::ClientServer::handle_accept(int sock) {
  using namespace tt::chat;
  ...
  if (read_size > 0) {
    ...
    std::cerr << "Read error on client socket " << socket_ << "\n";
  }
  close(sock);
}
